...
int16_t currentPitchBend=16384;


#define SEMITONE 1.0594631
#define TONE     1.122462
    ...
    uint8_t channel)
{
    ...
    if(inst->voice != NONE) {
        ...
        if(inst->voice==FM || inst->sync_voice) {
        ...
        SID_Set_Reg(SID_V1_CONTROL+offset, inst->voice|(inst->sync_voice?2:0), 
            sid_num);
    }

    // Check if a second voice is present and, if yes, play it.
    if(inst->voice2 != NONE || inst->voice==FM || inst->sync_voice) {
        if(inst->voice==FM || inst->sync_voice) {
